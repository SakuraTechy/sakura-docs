<template>
  ...
  >
    ...
    <div class="controls-overlay" @click="togglePlay">
      <div class="center-controls" v-if="!isPlaying">
...
const onDrag = (event) => {
  ...
  if (isDragging.value) {
    ...
    videoRef.value.currentTime = clampedPos * videoRef.value.duration
  } else if (event.type === 'mousemove') {
    isHovering.value = true
    const rect = event.currentTarget.getBoundingClientRect()
    const pos = (clientX - rect.left) / rect.width
    const clampedPos = Math.max(0, Math.min(1, pos)) * 100
    hoverPosition.value = clampedPos
    previewTime.value = (clampedPos / 100) * videoRef.value.duration
  }
}
